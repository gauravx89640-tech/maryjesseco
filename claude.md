...
- **Pages (real routes, not anchors):** `/` (Home), `/menu`, `/about`, `/gallery`, `/contact` — shared nav/footer via `app/layout.tsx`. Reviews (verbatim, real, in `lib/site-data.ts`): Annie Shanahan, Aubree, Gabrielle, Irma Thrashed (note: `lib/site-data.ts` currently misspells as "Irma Thrasher" — needs correcting), Rachelle.
