...
- **Gallery captions (`app/gallery/page.tsx` `PHOTOS`, corrected per Mary 2026-07-30):** `facial-treatment.jpg` = In Treatment · `headspa.jpeg` = Head Spa Ritual · `led.jpeg` = Scalp Analysis · `scalp.jpeg` = Custom Facial · `IMG_6267.jpeg` = The Details. Grid now holds **8 tiles**: `led-therapy.jpg` (LED Light Therapy, added + optimised 2026-07-30) plus **2 "Photo Coming Soon" placeholders** (`src: null`), both labelled **Rhonda Allison Skincare**, awaiting her still + video. When a photo lands, set its `src` and drop the `placeholder` flag. Note: 8 tiles in the `md:grid-cols-3` grid leaves one empty cell in the last desktop row — switch to 4 columns or add a 9th tile if that bothers her.
