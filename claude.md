...
- **Gallery captions (`app/gallery/page.tsx` `PHOTOS`, corrected per Mary 2026-07-30):** `facial-treatment.jpg` = In Treatment · `headspa.jpeg` = Head Spa Ritual · `led.jpeg` = Scalp Analysis · `scalp.jpeg` = Custom Facial · `IMG_6267.jpeg` = The Details. Grid holds **8 tiles and no placeholders** (2026-07-30): the five above plus `dermaplane.jpeg` (Dermaplane), `led-therapy.jpg` (LED Light Therapy), and `PY_Family_Photo.png` (Rhonda Allison Pro Youth). The product tile carries `wide: true` so it spans 2 columns on desktop — 7 tiles + 1 double = 9 cells = a clean 3×3, and 4 even rows of 2 on mobile. The `placeholder` branch (`src: null` → "Photo Coming Soon") is still in the component for future client media, just unused.
