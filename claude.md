...
- **Gallery captions (`app/gallery/page.tsx` `PHOTOS`, corrected per Mary 2026-07-30):** `facial-treatment.jpg` = In Treatment · `headspa.jpeg` = Head Spa Ritual · `led.jpeg` = Scalp Analysis · `scalp.jpeg` = Custom Facial · `IMG_6267.jpeg` = The Details. Grid now holds **9 tiles — a clean 3×3 with no trailing gap** (the earlier 8-tile layout left an empty cell): the five above plus `dermaplane.jpeg` (Dermaplane), `led-therapy.jpg` (LED Light Therapy), `PY_family_photo.png` (Rhonda Allison Pro Youth), and **one remaining "Photo Coming Soon" placeholder** (`src: null`, labelled Rhonda Allison Skincare) awaiting her second still/video. When a photo lands, set its `src` and drop the `placeholder` flag.
