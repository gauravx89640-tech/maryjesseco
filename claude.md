...
- **Client:** **Mary Jesse Skin + Scalp Studio** — note the **plus sign**, her explicit spelling (rebrand from "Mary Jesse & Co.", applied site-wide 2026-07-30: nav, footer, copyright, meta title/description, OG + Twitter tags, JSON-LD, image alt text, contact-form subject, map title). "Mary Jesse" alone still refers to her as a person (portrait alt text, review attribution) — do not rename those. Boutique esthetics studio, Arvada, CO. Owner: Mary. **ACTIVE CLIENT — preview stage, not live.** Go-live gated on final payment (amounts live in Desktop docs, never in this repo's content).
- **Promise line (Mary's own words, added 2026-09-15, verbatim):** "really good results + ridiculously good feels" — lives as a thin soft-sand strip on Home, between the hero and the wine menu band. The `+` is set upright in Dusty Rose `#A97D82` to echo "Skin + Scalp Studio". She also sent a second half ("come get on my table 👇" + booking link); Gaurav's call was **first part only, skip the rest** — do not add it back.
...
- **Service menu (CONFIRMED with Mary — full menu locked 2026-07-12; ordering is her explicit request):**
  - **Menu order (SUPERSEDED 2026-07-30 — Signature Facial now FIRST, Head Spa moved below it).** This reverses her earlier "list the Headspa Treatments up high as they are my money makers" instruction; the newer request wins. Live order: The Foundation (Signature) → add-on tiers → Teen Facial → The Crown (Head Spa) → Brows & Waxing → LED → Foot Reflexology → Custom Service → closer.
  - **Head Spa named tiers** (confirmed with Mary 2026-07-30): **👑 The Crown** — 60 min — $150 · **👑 The Royal Crown** — 90 min — $200 ("Everything in The Crown, plus extended massage, eye treatment, and lip treatment.")
  - **The Foundation / The Crown pairing (Mary's idea, locked):** Signature Facial = **The Foundation**, tagline *"Healthy skin starts with a strong foundation."* · Japanese Head Spa = **The Crown**, tagline *"Because your crown deserves the same care as your skin."* The Head Spa already contains the Signature Facial, so The Foundation literally lives inside The Crown — intentional, never spelled out on the page.
  - **Head Spa hero copy (Mary-approved, verbatim):** "A facial for your scalp—designed to deeply cleanse, nourish, and restore the foundation of healthy hair while melting away stress and tension." Secondary line beneath it: "Your customized scalp wellness experience designed to deeply cleanse, nourish, and restore your scalp while creating healthier, more beautiful hair."
  - **Every Head Spa includes (verbatim, 10):** Personalized scalp analysis · Essential oil welcome ritual · Deep scalp cleanse + exfoliation · Detoxifying scalp treatment · Nourishing shampoo + conditioning mask · Steam therapy for deep hydration · Hair growth serum + scalp elixir · Signature facial · Heated eye mask · Relaxation massage with specialty scalp tools. **The 90 min also includes:** Extended massage · Hydrating lip treatment · Brightening eye treatment.
  ...
  - **LED Therapy:** single 30 min — $50 · package of 3 — **$120** (updated 2026-07-30 from her tweaks list; supersedes the earlier $135 — Gaurav confirmed the newer figure wins)
  - **Foot Reflexology:** 30 min — $45 · 45 min — $60 · 60 min — $90
  - **Foot Spa (NEW, added 2026-09-15 — Head Spa techniques applied to the feet, incl. the waterfall feature):** 30 minutes — **$60**. Tagline *"Because your feet deserve a facial too."* (deliberately mirrors The Crown's). Description verbatim in `FOOT_SPA` in `lib/site-data.ts`. Rendered as a third dark anchor block (`#4A403B`) directly after The Crown, since Mary framed it as that service's sibling. **Not bookable until Mary returns from knee-replacement surgery in October 2026** — the block carries a "New · Starting October" badge and a "Booking opens when Mary returns in October" line **instead of a Book CTA**, so nobody books a service she cannot perform (and which may not exist in GlossGenius yet). When she's back: add the Book button and drop the badge/note. **Open question for Mary:** Foot Spa 30 min ($60) sits alongside Foot Reflexology 30 min ($45) — confirm both stay listed, since Foot Spa already includes reflexology.
...
- **Gallery captions (`app/gallery/page.tsx` `PHOTOS`, corrected per Mary 2026-07-30):** `facial-treatment.jpg` = In Treatment · `headspa.jpeg` = Head Spa Ritual · `led.jpeg` = Scalp Analysis · `scalp.jpeg` = Custom Facial · `IMG_6267.jpeg` = The Details. Grid holds **8 tiles and no placeholders** (2026-07-30): the five above plus `dermaplane.jpeg` (Dermaplane), `led-therapy.jpg` (LED Light Therapy), and `PY_Family_Photo.png` (Rhonda Allison Pro Youth). **All 8 tiles are the same size on purpose.** A 2-column span for the product tile was tried on 2026-07-30 and reverted: it made the supplier's catalog shot 599k px² against 279k for every photo of Mary working — more than double — inverting the hierarchy in a grid titled "the work, up close", and it upscaled the 850px source to 1.09× native. Uniform tiles leave one empty cell in the last desktop row; that gap is deliberate and preferable. Do not "fix" it by enlarging a tile. The `placeholder` branch (`src: null` → "Photo Coming Soon") is still in the component for future client media, just unused.
- **Gallery tile fit:** `PHOTOS` entries accept `contain: true` for wide or transparent art that must not be cropped — used by `PY_Family_Photo.png` (a transparent product lineup, 850×574), which letterboxes on the tan tile so the whole Pro Youth range stays visible. Everything else uses `object-cover`. **Known budget exception:** that PNG is 398KB vs the 300KB convention — transparency rules out JPEG and neither `pngquant` nor ImageMagick is installed on this Mac; `brew install pngquant` would bring it to roughly 120KB in one pass. Original preserved in the session scratchpad.
- **Brand lockup (changed 2026-07-30):** nav and footer now use a **stacked, all-lowercase** lockup — "mary jesse" in Cormorant over "skin + scalp studio" in Jost with `0.22em` tracking, centred in the nav, left-aligned in the footer column. Deliberately not uppercase: Gaurav's call that lowercase reads softer and suits "Skin + Scalp" better than all caps.
- **Menu colour system (built 2026-07-30 for Mary to react to; all values in the `C` map at the top of `app/menu/page.tsx` — change one line to restyle a category):** every category is its own solid colour block, styled like the dark Signature box she loves. Two dark anchors: Signature/The Foundation `#3E3833` (her "black") and Head Spa/The Crown `#6E4A50` (mauve), each with a light includes panel beside it (`#EFE9DE` / `#F3E7E4`). Add-on tiers run a cream→taupe ramp `#FAF6EF · #EFE9DE · #E5DFD3 · #DAD3C4`. Supporting categories: Teen Facial `#F3E7E4`, Brows & Waxing `#EFE9DE`, LED `#DAD3C4`, Foot Reflexology `#E4D2D0`, Custom Service outlined on cream, closer `#4A403B`.
- **SEO (added 2026-07-30 — previously absent entirely):** `app/layout.tsx` now carries metadataBase, canonical, OpenGraph + Twitter tags, and one JSON-LD `BeautySalon` schema with real NAP + hours. Deliberately omitted: `geo` coordinates and `aggregateRating` — both would need invented values (conventions 1 and 8). `SITE_URL` is still the Pages preview domain. **Mary bought `maryjesseskinandscalp.com` (registrar: Porkbun) on 2026-09-15, but as of that date it is NOT connected** — it 302s to a Porkbun parking page (`maryjesseskinandscalp-com.l.ink`), DNS points at Porkbun IPs (207.207.210.x), not Cloudflare. `SITE_URL` was deliberately NOT switched: canonical + OG pointing at a parking page is worse for SEO than the preview URL. **Order of operations:** connect the domain in Cloudflare Pages (Custom domains → add apex + www, repoint nameservers/DNS at Cloudflare) → confirm it serves the site over HTTPS → then flip `SITE_URL` and redeploy.
- **Open items:** (1) Media owed by Mary — the LED service description. Gallery has no placeholders left. Delivered 2026-07-30: hero + About portraits, dermaplane shot, Rhonda Allison Pro Youth lineup, and the door video (now `studio-lobby.mp4`, labelled "The Arrival"). (2) `app/contact/page.tsx` still has `WEB3FORMS_ACCESS_KEY = 'YOUR_WEB3FORMS_ACCESS_KEY'` — a literal placeholder, so the contact form sends nowhere; swap to Gaurav's key for preview, client's inbox only at go-live. (3) `@cloudflare/next-on-pages` is npm-deprecated (pinned to 1.13.16 to fix an ERESOLVE build break) — fine for now, will need a migration eventually.
- **Status:** closing stage — installment plan agreed, first installment invoiced; go-live locks when the FINAL payment clears (amounts and schedule live in Desktop docs, never in this repo). Full service menu confirmed — no pending prices remain (LED is $50 / 3 for $120). Web3Forms stays on Gaurav's key until the go-live checklist.
